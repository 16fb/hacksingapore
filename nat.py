...
# Display the fetched data for debugging purposes
st.dataframe(data)

# Convert fetched data into a usable format
job_database = {
    row[0]: {"skills": row[5].split(", "), "location": (row[3], row[4])}
...
if st.button('Search'):
    ...
    if user_location:
        st.write(f"User location coordinates: {user_location}")  # Debug: print user location coordinates
        ...
        st.write(f"Matched jobs based on skills: {matched_jobs}")  # Debug: print matched jobs
        ...
        st.write(f"Nearby jobs within 10 km: {nearby_jobs}")  # Debug: print nearby jobs
